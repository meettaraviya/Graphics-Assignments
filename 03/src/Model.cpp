...
void Model::fromFile(char* filename){
  ...
  int first_part,second_part,joint_type;
  ...
  for(int i=0;i<no_of_parts-1;i++){
    fscanf(inpFile,"%d %d %f %f %f",&first_part,&second_part,&joint_type,&contact_point[0],&contact_point[1],&contact_point[2]);
    Joint* joint = new Joint(&parts[first_part],&parts[second_part],contact_point);
    parts[first_part].addJoint(&parts[second_part]);
  }
}
